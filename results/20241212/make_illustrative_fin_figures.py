...
def curate_pec_fins(root, experiment_date, seg_model, seg_type, label_opacity=0.5, well_num=None, time_int=None):
    ...
    well_num = curation_df.loc[global_iter_i, "well_index"]
    time_int = curation_df.loc[global_iter_i, "time_index"]
    point_prefix = experiment_date + f"_well{well_num:04}" + f"_time{time_int:04}"
    ...
    # df
    df_name = file_prefix + f"_time{time_int:04}_fin_data.csv"
    fin_df = pd.read_csv(processed_data_path + df_name)
    fin_nucleus_ids = fin_df["nucleus_id"].to_numpy()

    print("Generating fin mask...")
    pd_mask = np.zeros(mask_zarr.shape, dtype=np.float32)
    pd_mask[np.isin(mask_zarr, fin_nucleus_ids)] = 1

    # get fin-specific layers
    print("Generating float fin mask...")
    prob_zarr_fin = prob_zarr.copy()
    # pd_mask_float = np.multiply(prob_zarr, pd_mask)
    ...
    # pd_mask[np.isin(mask_zarr, fin_nucleus_ids)] = 2

    prob_zarr_fin = np.multiply(prob_zarr_fin, pd_mask)
    ...
    prob_layer_fin = viewer.add_image(prob_zarr_fin, name="probabilities (fin region)", colormap="gray", scale=scale_vec, visible=False,
                                      contrast_limits=(-4, 100))
    # show tissue predictions
    # viewer.add_labels(mask_zarr, scale=scale_vec, name='nuclei masks (static)', opacity=1,
    #                       visible=False)

    prob_layer_fin2 = viewer.add_image(prob_zarr_fin, name="probabilities (fin region 2)", colormap="gray",
...
if __name__ == '__main__':
    ...
    experiment_date = "20240711_01"   #"20240712_02"
    overwrite = True
    fluo_flag = False
