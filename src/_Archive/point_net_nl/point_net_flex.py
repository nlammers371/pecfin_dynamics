...
class PointNetBackbone(nn.Module):
    ...
    ''' 
    def __init__(self, num_point_dims=3, num_points=2500, num_global_feats=128, local_feat=True):
        ''' Initializers:
        ...
        self.num_point_dims = num_point_dims
        self.num_points = num_points
        self.num_global_feats = num_global_feats
        self.local_feat = local_feat

        # Spatial Transformer Networks (T-nets)
        self.tnet1 = Tnet(dim=num_point_dims, num_points=num_points)
        self.tnet2 = Tnet(dim=64, num_points=num_points)

        # shared MLP 1
        self.conv1 = nn.Conv1d(num_point_dims, 64, kernel_size=1)
...
# ============================================================================
# Regression for predicting gene expression levels
class PointNetRegHead(nn.Module):
    ''' Regression Head '''
    def __init__(self, num_points=2500, num_point_features=0, num_global_feats=512, num_local_features=64, m=1):
        ...
        # shared MLP
        num_features = num_global_feats + num_local_features + num_point_features# local and global features
    ...
    def forward(self, x):
        ...
        xpos = x[:, 0:3] # run only positions through the main backbone
        xpos, crit_idxs, A_feat = self.backbone(xpos)

        # add other features
        xfull = torch.cat((xpos, x[:, 3:]), dim=1)

        # pass through shared MLP
        x0 = self.bn1(F.relu(self.conv1(xfull)))
        x1 = self.bn2(F.relu(self.conv2(x0)))
        x2 = self.bn3(F.relu(self.conv3(x1)))
        x3 = self.conv4(x2)

        x3 = x3.transpose(2, 1)
        # xmin, _ = torch.min(x2, dim=1, keepdim=True)
        # xmax, _ = torch.max(x2, dim=1, keepdim=True)
        # x2 = torch.divide((x2 - xmin), (xmax - xmin))
        
        return x3, crit_idxs, A_feat, x2
