...
data_capped = data_capped[:, 350:590, 75:300]
...
z_slice_vec = [87, 121, 70, 105]
x_slice_vec = [110, 120, 140]
y_slice_vec = [126]

for i in range(len(z_slice_vec)):
    z_slice_log = np.squeeze(data_log_i[z_slice_vec[i], :, :])
    io.imsave(os.path.join(train_path_log, f"well{well_int:03}_time{time_int:03}_xy_slice{i:03}.tiff"), z_slice_log, check_contrast=False)
    z_slice_bk = np.squeeze(data_rs[z_slice_vec[i], :, :])
    io.imsave(os.path.join(train_path_bk, f"well{well_int:03}_time{time_int:03}_xy_slice{i:03}.tiff"), z_slice_bk, check_contrast=False)

for i in range(len(x_slice_vec)):
    x_slice_log = np.squeeze(data_log_i[:, :, x_slice_vec[i]])
    io.imsave(os.path.join(train_path_log, f"well{well_int:03}_time{time_int:03}_zy_slice{i:03}.tiff"), x_slice_log, check_contrast=False)
    x_slice_bk = np.squeeze(data_rs[:, :, x_slice_vec[i]])
    io.imsave(os.path.join(train_path_bk, f"well{well_int:03}_time{time_int:03}_zy_slice{i:03}.tiff"), x_slice_bk, check_contrast=False)

for i in range(len(y_slice_vec)):
    y_slice_log = np.squeeze(data_log_i[:, y_slice_vec[i], :])
    io.imsave(os.path.join(train_path_log, f"well{well_int:03}_time{time_int:03}_zx_slice{i:03}.tiff"), y_slice_log, check_contrast=False)
    y_slice_bk = np.squeeze(data_rs[:, y_slice_vec[i], :])
    io.imsave(os.path.join(train_path_bk, f"well{well_int:03}_time{time_int:03}_zx_slice{i:03}.tiff"), y_slice_bk, check_contrast=False)
