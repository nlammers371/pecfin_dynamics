...
raw_data_directory = "E:\\Nick\\Dropbox (Cole Trapnell's Lab)\\Nick\pec_fin_dynamics\\fin_morphodynamics\\raw_data\\"
date_folder = "20230830"
image_directory = os.path.join(raw_data_directory, date_folder, '')


suffix_vec = ["_xy", "_zx", "_zy"]
window_size = 512
decon_flag = True
n_samples = 2  # number of samples in xy and zx/zy directions (total num samples per image = 2*n_samples)
...
# set write path
write_path = os.path.join(raw_data_directory, 'cellpose_training_slices', '')
...
for im in range(len(image_list)):
    ...
    for well_index in range(n_wells):
        ...
        for t in range(n_time_points):
            # extract image
            image_data = np.squeeze(imObject.get_image_data("CZYX", T=t))
            ...
            for image_i in range(len(slice_id_vec)):
                ...
                out_name = os.path.join(write_path, f'{rand_prefix:06}' + '_' + save_name + suffix + f'{slice_num:03}')
