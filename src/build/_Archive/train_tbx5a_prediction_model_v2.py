...
def train_point_net_reg(root, training_dates, fluo_channel, model_name, num_epochs=100, learning_rate=0.0001,
                        point_cloud_size=4096, batch_size=32, train_frac=0.8, point_features=None):
    ...
    save_path = os.path.join(root, "built_data", "nucleus_data", "point_models_" + model_name, str(timestamp))
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    # generate dataloader to load fin point clouds
    point_data = PointDataReg(data_root, split='train', training_dates=training_dates, point_features=point_features,
...
if __name__ == '__main__':
    ...
    train_point_net_reg(root, training_dates=experiment_dates, fluo_channel=fluo_channel, model_name=model_name,
                        point_features=[], num_epochs=250)