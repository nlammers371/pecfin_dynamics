...
for e, experiment_date in enumerate(experiment_date_vec):
    ...
    mask_stack_list = sorted(glob(os.path.join(root, "built_data", "mask_stacks", seg_model, experiment_date, "*mask_stacks.zarr")))

    for m, zarr_path in enumerate(tqdm(mask_stack_list[4:], "Restitching masks...")):
